...
void PluginLoader::loadPlugins(const string& pluginDir) {
    if (!fs::exists(pluginDir) || !fs::is_directory(pluginDir)) {
        ...
        throw runtime_error("Package of plugins wasn't founded " + pluginDir);
    }
                ...
                if (creator) {
                    ...
                    if (pluginPtr) {
                        ...
                        if (plugins.count(name)) {
                            ...
                            cerr << "Plugin '" << name << "' already loaded. Skip." << endl;
                            delete pluginPtr;
                        }
                        else {
                        ...
                        cerr << "Error  createPlugin returned nullptr for " << path << endl;
                    }
                }
                else {
                    
                    cerr << "Error, not found function 'createPlugin' in " << path << endl;
                }
            }
            else {
                
                cerr << "Error, can't load library " << path << endl;
            }
        }
    }
}
