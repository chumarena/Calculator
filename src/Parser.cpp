...
bool Parser::is_operator(const string& token) {
    ...
    auto it = plugins.find(token);
    return it != plugins.end() && it->second->getType() == OpType::BINARY;
}
    ...
    for (int i = 0; i < expression.length(); ++i) {
        ...
        if (isdigit(c) || c == '.') {
            ...
            if (token == "(" || token == ")") {
                ...
                throw runtime_error("Unavailable symbol in: " + token);
            }
        }
    }
    return tokens;
...
vector<string> Parser::infix_to_rpn(const vector<string>& tokens) {
    ...
    for (int i = 0; i < tokens.size(); ++i) {
        ...
        if (is_number(token)) {
            ...
            if (op_stack.empty()) throw runtime_error("Uncoordinated brackets.");
            op_stack.pop();
    ...
    while (!op_stack.empty()) {
        if (op_stack.top() == "(" || op_stack.top() == ")") {
            throw runtime_error("Uncoordinated brackets.");
        }
        rpn_output.push_back(op_stack.top());
        op_stack.pop();
    }
